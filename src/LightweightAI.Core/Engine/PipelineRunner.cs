...
public class PipelineRunner : IPipelineRunner
{
    ...
    public PipelineRunner()
    {
        ...
        if (File.Exists(fusionConfigPath))
        {
            Dictionary<string, double>? cfg =
                JsonSerializer.Deserialize<Dictionary<string, double>>(File.ReadAllText(fusionConfigPath));
    ...
    private IEnumerable<EventContext> LoadEventsFromPs()
    {
        ...
        return JsonSerializer.Deserialize<List<EventContext>>(json, new JsonSerializerOptions
        {
