...
public class RulesEngine
{
    ...
    /// <summary>
    ///     Adds a rule with optional priority (higher evaluated first) and severity hard floor (minimum fused score if
    ///     matched).
    /// </summary>
    public void AddRule(IRule rule, int priority = 0, double? hardFloor = null)
    {
        this._rules.Add((rule, priority, hardFloor));
        this._rules.Sort(static (a, b) => b.priority.CompareTo(a.priority));
    }
    ...
    {
        foreach ((IRule rule, int priority, double? hardFloor) entry in this._rules)
    ...
    {
        foreach ((IRule rule, var _, var hardFloor) in this._rules)
        {
            if (hardFloor is null) continue;
            RuleResult res = rule.Evaluate(ctx);
