...
public class ProvenanceLog
{
    ...
    // Provide overload to match training namespace variant signature
    ...
    public void PruneNonEssential(Guid envelopeId, TimeSpan ageThreshold, IProvenanceArchive archive)
    {
        ...
        var toArchive = this._entries.Where(e => e.Importance == ProvImportance.Verbose && e.TimestampUtc < cutoff).ToList();
