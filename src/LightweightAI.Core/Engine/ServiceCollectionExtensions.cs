...
using LightweightAI.Core.Engine.Pipeline;
...
public static class ServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddCoreContracts(this IServiceCollection services)
    {
        ...
        services.AddSingleton<IAlertSink>(sp => new SyslogAlertSink("127.0.0.1"));
        services.AddSingleton<IAlertSink>(sp => new WebhookAlertSink("http://localhost:5001/alert"));
        // Smtp optional (example values)
        services.AddSingleton<IAlertSink>(sp =>
            new SmtpAlertSink("localhost", 25, "alerts@local", "ops@local"));
        services.PostConfigure<AlertDispatcher>(d =>
        {
            ServiceProvider serviceProvider = services.BuildServiceProvider();
            foreach (IAlertSink sink in serviceProvider.GetServices<IAlertSink>())
                d.AddSink(sink);
        });
