...
{
    public static IEnumerable<T> WeightedSample<T>(IEnumerable<T> items, Func<T, string> labelSelector, int take, float minorityBoost = 2f)
    {
        var groups = items.GroupBy(labelSelector).ToDictionary(g => g.Key, g => g.ToList());
        ...
        foreach (var kv in groups)
        {
            ...
            for (int i = 0; i < effective && kv.Value.Count > 0; i++)
