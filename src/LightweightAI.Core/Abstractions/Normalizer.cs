...
{
    private static readonly HashSet<string> SeverityValues = new(StringComparer.OrdinalIgnoreCase) { "Verbose", "Info", "Warn", "Error", "Critical" };
    ...
    {
        var fields = new Dictionary<string, object?>(input.Fields, StringComparer.OrdinalIgnoreCase);

        // Coerce timestamp drift (future skew > 5 min)
        var ts = input.TimestampUtc > DateTimeOffset.UtcNow.AddMinutes(5) ? DateTimeOffset.UtcNow : input.TimestampUtc;
        ...
        foreach (var k in fields.Keys.ToList())
        {
            ...
            if (v is string s)
            {
                if (int.TryParse(s, out var i) && (ProcessIdKeys.Contains(k, StringComparer.OrdinalIgnoreCase) || PortKeys.Contains(k, StringComparer.OrdinalIgnoreCase)))
                {
                    fields[k] = i;
                    continue;
                }
