        ...
        CancellationToken ct)
    {
        foreach (MemoryAnomaly a in anoms)
        {
            TimelineEvent ev = await MakeEventAsync(
                ...
                a.LastSeen,
                DateTime.UtcNow,
                $"Memory anomaly in {a.ProcessName} at {a.BaseAddress}: {string.Join((string?)(string?)", ", (IEnumerable<string?>)a.RulesTriggered)}",
                new Dictionary<string, string>
                {
                    ["Pid"] = a.Pid.ToString(),
                    ...
                    ["Rules"] = string.Join((string?)(string?)"|", (IEnumerable<string?>)a.RulesTriggered)
                },
                MinimalJson(a),
                ct);
            await store.AppendAsync(ev, ct).ConfigureAwait(false);
